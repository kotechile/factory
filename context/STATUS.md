...
`context/vertical_coverage.md`. The factory's 26 audience verticals come from the editorial registry;
the ledger's inventory block is generated by `scripts/vertical-sync.mjs` (`--check` fails on stale
inventory or a missing/unknown verdict row, so the two registries cannot drift apart silently). Reason:
4 of 5 PRDs sat in one vertical cluster (money/document reconciliation) while the archetypes were
